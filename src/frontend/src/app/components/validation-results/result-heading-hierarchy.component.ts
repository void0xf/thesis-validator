...
@Component({
  ...
  template: `
    @if (headings.length > 0) {
      <div class="paper-card results-surface overflow-hidden animate-slide-up">
        <div class="px-5 py-4 flex items-center gap-4">
          <div
            class="w-10 h-10 rounded-xl flex items-center justify-center bg-academic-green/15 text-academic-green"
          >
            ...
            @for (heading of headings; track heading) {
              ...
              >
                <span
                  ...
                  [class]="levelClasses[heading.level] || defaultLevelClass"
                >
                  H{{ heading.level }}
                </span>
                <span
...
export class ResultHeadingHierarchyComponent {
  ...
  readonly levelClasses: Record<number, string> = {
  ...
  readonly defaultLevelClass = 'bg-academic-red/15 text-academic-red';
}
