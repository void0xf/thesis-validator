...
public static partial class TableOfContentsDetectionService
{
    ...
    public static TableOfContentsDetection Detect(WordprocessingDocument doc)
    {
        ...
        foreach (var paragraph in body.Descendants<Paragraph>())
        {
            ...
            if (ContainsTocFieldCode(paragraph))
            {
                return new TableOfContentsDetection(
                    ...
                    paragraph,
                    paragraphIndex,
                    TextExtractionService.GetParagraphText(paragraph, skipTextBoxes: false).Trim());
            }
        }
    ...
    private static TableOfContentsDetection? FindManualTableOfContents(Body body)
    {
        ...
        {
            paragraphIndex++;
            var text = TextExtractionService.GetParagraphText(paragraph, skipTextBoxes: false);
            if (string.IsNullOrWhiteSpace(text))
                continue;
