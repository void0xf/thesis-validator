...
{
    private readonly ModernDocumentSkipService _skipService;

    public DocumentContentAnalyzer(ModernDocumentSkipService? skipService = null)
    {
        _skipService = skipService
            ?? new ModernDocumentSkipService(Options.Create(new ModernValidationOptions()));
    }
    ...
    public DocumentContent Analyze(WordprocessingDocument doc)
    {
        ...
        foreach (var element in body.ChildElements)
        {
            ...
            var text = TextExtractionService.GetParagraphText(
                paragraph,
