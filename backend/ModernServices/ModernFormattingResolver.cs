...
public sealed class ModernFormattingResolver
{
        ...
        Run run)
    {
        ...
        var defaultStyleSize = UnitConversion.HalfPointsToPoints(
            StyleResolutionService.GetDefaultParagraphStyle(document)?.StyleRunProperties?.FontSize?.Val?.Value);
        if (defaultStyleSize is not null)
            return defaultStyleSize;

        return UnitConversion.HalfPointsToPoints(
            StyleResolutionService.GetDocumentDefaultRunProperties(document)?.FontSize?.Val?.Value);
    }
        ...
        Run run)
    {
        ...
        var defaultBold = GetOnOffValue(
            StyleResolutionService.GetDocumentDefaultRunProperties(document)?.Bold);
        ...
        Paragraph paragraph)
    {
        ...
        var defaultSpacing = ParseLineSpacing(
            StyleResolutionService.GetDefaultParagraphStyle(document)?
                .StyleParagraphProperties?
                .SpacingBetweenLines);
        ...
        return ParseLineSpacing(
            StyleResolutionService.GetDocumentDefaultParagraphProperties(document)?
                .SpacingBetweenLines);
    }
        ...
        Paragraph paragraph)
    {
        ...
        var defaultStyle = StyleResolutionService.GetDefaultParagraphStyle(document);
        ...
        foreach (var style in StyleResolutionService.GetStyleChain(document, defaultStyle?.BasedOn?.Val?.Value))
        {
            var styleIndentation = style.StyleParagraphProperties?.Indentation;
            if (HasExplicitFirstLineIndentation(styleIndentation))
                return GetFirstLineIndent(styleIndentation);
