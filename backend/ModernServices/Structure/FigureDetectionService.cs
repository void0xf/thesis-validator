...
namespace backend.ModernServices.Structure;

public static class FigureDetectionService
{
    public static bool ContainsFigureCandidate(Paragraph paragraph, bool skipTextBoxes = true)
    {
        if (paragraph.Descendants<Drawing>().Any(drawing => IsFigureObject(drawing, skipTextBoxes)))
            return true;

        return paragraph.Descendants<Picture>().Any(picture => IsFigureObject(picture, skipTextBoxes));
    }

    private static bool IsFigureObject(OpenXmlElement element, bool skipTextBoxes)
    {
        ...
        return !TextBoxContentDetector.ContainsTextBoxContent(element)
