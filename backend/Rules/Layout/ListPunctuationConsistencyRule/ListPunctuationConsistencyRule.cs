...
public sealed class ListPunctuationConsistencyRule : ValidationRule<ListPunctuationConsistencyRuleOptions>
{
    ...
    public ListPunctuationConsistencyRule(ListAnalyzer? listAnalyzer = null)
    {
    ...
    private static IEnumerable<RuleProblem> ValidatePunctuationConsistency(ListGroup list)
    {
    ...
    private static string GetListItemPreview(string text)
    {
        return string.IsNullOrWhiteSpace(text)
            ? "[empty]"
            : TextExtractor.Truncate(text, 40);
    }
}
