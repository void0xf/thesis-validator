...
public sealed class ModernRuleRunner
{
    private readonly IReadOnlyList<IModernValidationRule> _rules;
    ...
    public ModernRuleRunner(
        IEnumerable<IModernValidationRule> rules,
    ...
    public IReadOnlyList<RuleDefinition> GetAvailableRules()
    {
        return _rules
            ...
            .Select(pair => new RuleDefinition(
    ...
    public IReadOnlyList<ModernRuleExecution> Run(
        RuleContext context,
        IEnumerable<string>? selectedRules)
    {
        var executions = new List<ModernRuleExecution>();
        ...
        foreach (var rule in GetRulesToRun(selectedRules))
        {
            ...
            foreach (var problem in rule.Validate(context, boundOptions))
            {
                ...
                executions.Add(new ModernRuleExecution(result, problem));
            }
        }
    ...
    private IReadOnlyList<IModernValidationRule> GetRulesToRun(IEnumerable<string>? selectedRules)
    {
...
public sealed record ModernRuleExecution(
