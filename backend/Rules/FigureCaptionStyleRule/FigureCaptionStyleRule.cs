...
/// </summary>
public sealed class FigureCaptionStyleRule : ValidationRule<NoRuleOptions>
{
        ...
        NoRuleOptions options)
    {
        foreach (var captionCandidate in _figureCaptionAnalyzer.GetDetectedFigureCaptions(context))
        {
            ...
            if (!CaptionDetectionService.UsesDedicatedCaptionStyle(context.RawDocument, caption))
            {
                var label = CaptionDetectionService.GetCaptionStyleLabel(context.RawDocument, caption);
            ...
            var textRun = caption.Elements<Run>()
                .FirstOrDefault(run => !string.IsNullOrWhiteSpace(TextExtractionService.GetRunText(run)));
