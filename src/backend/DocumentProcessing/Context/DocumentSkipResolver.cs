...
public sealed class DocumentSkipResolver
{
    private readonly ValidationSkippingOptions _options;

    public DocumentSkipResolver(IOptions<ValidationSkippingOptions> options)
    {
        ...
        WordprocessingDocument doc)
    {
        ...
        var tocParagraph = TableOfContentsDetector.Detect(doc).Paragraph;
        ...
        WordprocessingDocument doc)
    {
        ...
        {
            if (TableOfContentsDetector.ContainsTocFieldCode(paragraph))
            {
                inTocField = true;
    ...
    private static bool IsTextBoxOnlyParagraph(Paragraph paragraph)
    {
        return paragraph.Descendants<Text>().Any(TextBoxContentDetector.IsInsideTextBoxOrDrawingText)
            && !TextExtractor.HasMeaningfulContent(GetParagraphTextWithoutTextBoxes(paragraph));
    }
